...
export const AgentAnalyticsTool: React.FC = () => {
  ...
  return (
    <div className="min-h-screen bg-gray-50 py-4 px-1 sm:px-4 lg:px-8 max-w-screen-lg mx-auto">
      <h1 className="text-2xl sm:text-3xl font-bold text-center mb-6 text-indigo-600">Agent Analytics Dashboard</h1>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mb-8">
        ...
        <div className="bg-white rounded-xl shadow p-3 flex flex-col">
            ...
            <div className="flex flex-wrap gap-1">
              {TIME_FILTERS.map(f => (
                <button
                  ...
                  className={`px-2 py-1 rounded-full text-xs font-semibold border transition-colors ${timeFilter === f.value ? 'bg-indigo-600 text-white border-indigo-600' : 'bg-white text-gray-700 border-gray-300 hover:bg-indigo-50'}`}
                >
                  {f.label}
                </button>
              ))}
          ...
          <div className="flex items-center gap-1 mb-2">
            ...
            <button
              className={`px-2 py-1 rounded-full text-xs font-semibold border transition-colors ${purchaseFilter ? 'bg-indigo-600 text-white border-indigo-600' : 'bg-white text-gray-700 border-gray-300 hover:bg-indigo-50'}`}
                ...
                <ul className="space-y-1">
                  {cat.products.slice(0, 10).map((p: any, idx: number) => (
                    <li key={p.name} className="flex items-center justify-between text-xs">
                      ...
                      <span className="font-bold text-indigo-600">{p.value}</span>
                    </li>
                  ))}
                </ul>
              </div>
