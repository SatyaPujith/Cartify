...
export const CartifyAssistant: React.FC = () => {
    ...
    try {
      ...
      recorder.onstop = async () => {
        ...
        try {
          ...
          // Try Raindrop backend first (LiquidMetal AI platform)
          if (raindropUrl) {
            response = await fetch(`${raindropUrl}/api/speech-to-text`, {
              method: 'POST',
              body: formData,
            });
          ...
          response = await fetch('https://cmpgbcxxekyjtvvcabbw.supabase.co/functions/v1/speech-to-text', {
            method: 'POST',
            body: formData,
            headers: {
  ...
  // Helper to call the Cartify AI Agent
  async function fetchCartifyAgentResponse(query: string) {
    ...
    try {
      // Try Raindrop backend first (LiquidMetal AI platform)
      if (raindropUrl) {
        const response = await fetch(`${raindropUrl}/api/cartify-agent`, {
          method: 'POST',
          headers: {
      ...
      const localResponse = await fetch('http://localhost:3001/api/cartify-agent', {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
        },
      ...
      // Final fallback to Supabase edge function
      ...
      console.error('Error calling Cartify agent:', error)
      throw error
    }
  }
  ...
  const addAllToCart = async () => {
    if (!user) {
      setMessages(prev => [...prev, { 
        type: 'bot', 
        content: `⚠️ Please sign in to add items to your cart. Click the "Sign In" button in the header to continue shopping!` 
      }])
      return
    }
    ...
    try {
      for (const product of suggestedProducts) {
        ...
        await addToCart(productForCart, product.quantity);
      }
      setMessages(prev => [...prev, { 
        type: 'bot', 
      ...
      setMessages(prev => [...prev, { 
        type: 'bot', 
        content: `❌ Sorry, there was an error adding items to your cart. Please try again.` 
      }])
    }
  }
  ...
  return (
    <div className="fixed inset-0 bg-black bg-opacity-50 z-50 flex items-center justify-center p-1 sm:p-2">
      <div className="bg-white rounded-2xl w-full max-w-screen-md h-[80vh] max-h-screen flex flex-col overflow-hidden shadow-2xl border-2 border-indigo-500">
        {/* Fixed Header */}
        <div className="bg-indigo-600 text-white px-3 py-2 sm:px-4 sm:py-3 flex items-center justify-between flex-shrink-0">
          <div className="flex items-center space-x-2 sm:space-x-3">
            <div className="w-8 h-8 sm:w-10 sm:h-10 bg-white bg-opacity-20 rounded-full flex items-center justify-center">
              <Bot className="h-5 w-5 sm:h-6 sm:w-6 text-white" />
            </div>
            <div>
              <h2 className="text-base sm:text-lg font-bold text-white">Cartify AI Assistant</h2>
              <p className="text-indigo-100 text-xs sm:text-sm">Your intelligent shopping assistant</p>
            </div>
          </div>
                  ...
                  {quickOptions.map((option) => (
                    <button
                      ...
                      onClick={() => handleQuickOption(option.id)}
                        className="p-2 border border-gray-200 rounded-xl hover:border-indigo-500 hover:bg-indigo-50 transition-all text-left group shadow bg-white"
                    >
                        <div className="flex items-start space-x-2">
                          <span className="text-lg sm:text-xl">{option.icon}</span>
                        <div>
                            <div className="font-bold text-xs sm:text-sm text-gray-900 group-hover:text-indigo-600">
                            {option.title}
                          </div>
                ...
                <div className="flex justify-start">
                    <div className="bg-white px-3 py-2 rounded-xl shadow border border-gray-200">
                      <div className="flex items-center space-x-2">
                        ...
                        <span className="text-xs text-gray-600 font-medium">Cartify AI is thinking...</span>
                    </div>
                  </div>
                </div>
              )}
              ...
              <div className="border-t bg-white p-2 sm:p-3 flex-shrink-0 shadow">
                ...
                <div className="flex-1 relative">
                  <input
                    ...
                    placeholder="Tell me what you need... (e.g., 'I want to cook biryani under $50')"
                      className="w-full px-3 py-2 border border-gray-300 rounded-full focus:outline-none focus:ring-2 focus:ring-indigo-500 focus:border-transparent text-xs sm:text-sm"
                ...
                <button
                  ...
                  disabled={!input.trim() || isProcessing}
                    className="bg-indigo-600 text-white p-2 rounded-full hover:bg-indigo-700 disabled:opacity-50 disabled:cursor-not-allowed transition-colors shadow"
                >
              ...
              <div className="w-full lg:w-2/5 xl:w-1/3 bg-gray-50 flex flex-col border-t lg:border-t-0">
                ...
                <button
                  onClick={addAllToCart}
                    className="w-full bg-emerald-500 text-white font-bold py-2 px-4 rounded-full hover:bg-emerald-600 transition-colors flex items-center justify-center space-x-2 shadow text-xs sm:text-sm"
                >
                ...
                <>
                  <div className="p-3 border-b bg-white flex-shrink-0 flex items-center justify-between">
                    ...
                    <button
                      ...
                      className="bg-emerald-500 text-white font-bold py-2 px-4 rounded-full hover:bg-emerald-600 transition-colors flex items-center space-x-2 shadow text-xs sm:text-sm"
